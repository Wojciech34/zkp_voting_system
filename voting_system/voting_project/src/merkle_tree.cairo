...
pub trait IMerkleTree<TContractState> {
    ...
    // verify if leaf node exists in merkle tree
    fn verify(
...
mod errors {
    pub const NOT_POW_2: felt252 = 'Data length is not power of 2';
    pub const NOT_PRESENT: felt252 = 'Not element in merkle tree';
}
...
pub mod MerkleTree {
    ...
    impl IMerkleTreeImpl of super::IMerkleTree<ContractState> {
        fn build_tree(ref self: ContractState, mut data: Array<ByteArray>) -> Array<felt252> {
            ...
            // assert len is positive and is power of 2
